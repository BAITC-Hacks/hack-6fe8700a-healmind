...
from backend.ai_analyst import AnalysisUnavailable, generate_analysis
...
def main():
    ...
    if "ai_analysis" not in evaluation and "ai_error" not in evaluation:
        with st.spinner("AI объясняет результаты…"):
            try:
                evaluation["ai_analysis"] = generate_analysis(decisions, result)
                evaluation.pop("ai_error", None)
            except AnalysisUnavailable as exc:
                evaluation["ai_error"] = str(exc)
    if evaluation.get("ai_analysis"):
        st.write(evaluation["ai_analysis"])
        st.caption("Разбор создан AI на основе расчёта. Проверяйте утверждения по таблице выше.")
    elif evaluation.get("ai_error"):
        st.warning(evaluation["ai_error"])
        st.caption("Score и таблица рассчитаны программой и доступны независимо от AI.")
        if st.button("Повторить AI-разбор", key="retry_ai"):
            evaluation.pop("ai_error", None)
            st.rerun()
